...
def encrypt_simhash(pubkey, x): # x: 64bit simhash
    if not isinstance(pubkey,AdditiveElgamalKey):
        raise ValueError('Argument pubkey must be AdditiveElgamalKey instance')
    if x>=2**64:
...
'''
privkey = construct_additive((key_json['p'], key_json['g'], key_json['y'],key_json['x']))
